...
const routerEmmiter = new EventEmitter;

const utils = require('./utils');


routerEmmiter.on('get/index', async (ctx, resolve, reject) => {
  try {
...
routerEmmiter.on('post/index', async ({ name, email, message }, resolve, reject) => {
  try {
...
routerEmmiter.on('post/login', ({ email, password }, resolve, reject) => {
  utils.setField('authorize', { email, password }, resolve, reject);
});

routerEmmiter.on('post_skills/admin', ({ age, concerts, cities, years }, resolve, reject) => {
  utils.setField('skills', { age, concerts, cities, years }, resolve, reject);
});

routerEmmiter.on('post_upload/admin', async (ctx, resolve, reject) => {
